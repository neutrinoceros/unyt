...
@pytest.mark.parametrize(
    "s, expected", [
        ...
        ("1e3km", 1e3 * Unit("km")),
        ("-1e3    km", -1e3 * Unit("km")),
        ("1.0 g/cm**3", 1. * Unit("g/cm**3")),
        ("1 g*cm**-3", 1. * Unit("g/cm**3")),
        ("1.0 g*cm", 1. * Unit("g*cm")),
...
def test_invalid_unit_quantity_from_string(s):
    ...
    un_str = s.split()[1]
    with pytest.raises(UnitParseError, match="Could not find unit symbol '{}' in the provided symbols.".format(un_str)):
        unyt_quantity.from_string(s)
